...
tSTorM32CmdGetVersionStr* t = (tSTorM32CmdGetVersionStr*)buf;
  ...
  t->stx = 0xFA; //0xF9 to suppress response
  t->len = 0x00;
  t->cmd = 0x02;
...
//------------------------------------------------------
// RCCMD  ActivePanModeSetting  0x64
//------------------------------------------------------
// only outgoing, has no response (except of an ACK)
// outgoing: X bytes = 1650us @ 115200bps

#define STORM32RCCMD_ACTIVE_PAN_MODE__SETTING_OUTLEN      0x01

struct STORM32LIBPACKED tSTorM32CmdActivePanModeSetting{ //len = 0x01
  uint8_t stx;
  uint8_t len;
  uint8_t cmd;
  uint8_t data;
...
{
tSTorM32CmdActivePanModeSetting* t = (tSTorM32CmdActivePanModeSetting*)buf;

  t->stx = 0xFA; //0xF9 to suppress response
  t->len = 0x01;
  t->cmd = 0x64;
  t->crc = crc_calculate(&(t->len), sizeof(tSTorM32CmdActivePanModeSetting)-3);
}
...
static inline void storm32_finalize_CmdActivePanModeSetting_(void* buf, uint16_t* len)
{
  ...
  *len = sizeof(tSTorM32CmdActivePanModeSetting);
}

//------------------------------------------------------
// RCCMD  StandBySetting  0x0E
//------------------------------------------------------
// only outgoing, has no response (except of an ACK)
// outgoing: X bytes = 1650us @ 115200bps
...
struct STORM32LIBPACKED tSTorM32CmdStandBySetting{ //len = 0x01
  uint8_t stx;
  uint8_t len;
  uint8_t cmd;
  uint8_t data;
...
static inline void storm32_finalize_CmdStandBySetting(void* buf)
{
tSTorM32CmdStandBySetting* t = (tSTorM32CmdStandBySetting*)buf;

  t->stx = 0xFA; //0xF9 to suppress response
  t->len = 0x01;
  t->cmd = 0x0E;
  t->crc = crc_calculate(&(t->len), sizeof(tSTorM32CmdStandBySetting)-3);
}


static inline void storm32_finalize_CmdStandBySetting_(void* buf, uint16_t* len)
{
  storm32_finalize_CmdStandBySetting(buf);
  *len = sizeof(tSTorM32CmdStandBySetting);
}
...
tSTorM32CmdSetPitchRollYaw* t = (tSTorM32CmdSetPitchRollYaw*)buf;
  ...
  t->stx = 0xFA; //0xFA; //0xF9 to suppress response
  t->len = 0x06;
  t->cmd = 0x12;
